...
// если удалять постом lumen ругается что запрос поститься, а данные постом не передаются. сделал гет
// В получение твитов надо как то передать limit. Тесть сделать постом и передавать параметр в теле, или добавить ище один гет параметр, что я и сделал
// ->were() В люмене не работает
$app->get('/accounts', 'AccountsController@listAccounts');
...
$app->get('/accounts/gettwitterposts', 'AccountsController@getTwitterPosts');//->where('account_id', '[0-9]+');